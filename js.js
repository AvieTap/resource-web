...
$(document).ready(function(){
    ...
    // Tried toggling via addClass()/removeClass() but they didn't properly override the window styles- kept getting overwritted AGAIN by the user inputs in the tabs, so it'd undo the toggle

    // Unfortunately need to manually change everything back and forth- will change if I find something more efficient

    
    // Toggle CSS in miniWindow FIX LATER
    $("#toggleCSS").click(function(){
        if (cssOFF == false) {
            cssOFF = true;
            $("#miniWindow, .ib, .collections, #lines, #lists, #media").css("background-color", "white");
    ...
    $("#toggleHTML").click(function(){
        if (htmlOFF == false) {
            ...
            $("#title, #h1, #h2, #p, #pi, #pb, #ul1, #ul2, #ul3, #ul4, #ul5, #ol1, #ol2, #ol3, #ol4, #ol5, #a, #input, #label, #warning, #button, #th1, #th2, #th3, #td1a, #td2a, #td3a, #td1b, #td2b, #td3b").html("");
            $("#img").attr("src", "");
            $("#a").attr("href", "");
            ...
            $("#p").html($("#text-p").val());
            let pItalicized = $("#text-pi").val();
            let result1i = pItalicized.replaceAll(" *", " <i class='p ib'>");
            let result2i = result1i.replaceAll("*", "</i>");    
            $("#pi").html(result2i);
            let pBolded = $("#text-pb").val();
            let result1b = pBolded.replaceAll(" **", " <b class='p ib'>");
            let result2b = result1b.replaceAll("**", "</b>");
            $("#pb").html(result2b);
